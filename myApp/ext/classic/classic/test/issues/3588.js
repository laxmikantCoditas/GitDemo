...
function test(){
    Ext.onReady(function(){
        Ext.require('Ext.ComponentQuery', function(){
            ...
            var scrollerCount = Ext.query('.x-scroller').length;
            if (scrollerCount != 2) {
                throw Error('Test case expected 2 scrollers');
            }

            Ext.Array.forEach(Ext.ComponentQuery.query('gridcolumn[locked]'), function(component){
                
                var xy = [];
                xy[0] = component.el.getRight();
                xy[1] = component.el.getBottom();
                
                fuzzy.clickElement(component.triggerEl.dom, xy);
                
                Ext.Array.forEach(Ext.ComponentQuery.query('menuitem[text=Unlock]'), function(component){
                    fuzzy.clickElement(component.el.dom);
                });
                
                setTimeout(function(){
                    if (Ext.query('.x-scroller').length < scrollerCount) {
                        throw Error('FAIL: ' + Ext.versions.extjs.version + ' EXTJSIV-3588 -- scrollbar disappears in locking grid');
                    } else {
                        console.log('PASS: ' + Ext.versions.extjs.version + ' EXTJSIV-3588 -- scrollbar disappears in locking grid');
                    }
                }, 0.5e3);
            });
...
fuzzy.hoverElement = function(target, xy, callback){
    var event = new Ext.EventObjectImpl({target:target.dom || target, xy:xy});
    event.xy = xy;
    event.type = 'mouseenter';event.injectEvent();
    event.type = 'mouseover';event.injectEvent();
