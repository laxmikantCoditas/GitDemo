describe("jasmine.panel.Infos", function() {
    ...
    describe("constructor", function() {
        beforeEach(function() {
            spyOn(jasmine.panel.Infos.prototype, "renderFailedResult").andCallThrough();
            panel = new jasmine.panel.Infos({
                spec: spec
            });
        });
        ...
        it("should call renderResultMessage for each failed results", function() {
            expect(jasmine.panel.Infos.prototype.renderFailedResult.calls.length).toEqual(1);
        });
    });
    ...
    describe("renderFailedResult", function() {
        ...
        if (!jasmine.browser.isIE) {
            it("should add a children if result contains error", function() {
                expect(panel.renderFailedResult(result2).childNodes.length).toEqual(2);
            });
        }
    });
